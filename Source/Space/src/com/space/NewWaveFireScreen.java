	...
	public void render(float delta) {
		...
		if(!Gdx.input.isTouched()){
			...
			if (selector.overlaps(upgrade1Rect)){
				...
				Space.player.upgradeFireDuration();
				selector.x = -400;
				selector.y = -240;
			}
			if (selector.overlaps(upgrade2Rect)){
				//Upgrade Fireball Damage
				Space.player.upgradeFireballDamage();
				selector.x = -400;
				selector.y = -240;
			}
			if (selector.overlaps(upgrade3Rect)){
				//Upgrade FireRate
				Space.player.upgradeFireballFireRate();
				selector.x = -400;
				selector.y = -240;
			}
			if (selector.overlaps(upgrade4Rect)){
				//Upgrade Health
				Space.player.upgradeFireballVelocity();
				selector.x = -400;
				selector.y = -240;
			}
		}
		...
		if (currentSelection == 7||selector.overlaps(upgrade1Rect)){
			batch.draw(upgradeSelectionImg,  167, 336);
		}
		if (currentSelection == 8||selector.overlaps(upgrade2Rect)){
			batch.draw(upgradeSelectionImg,  167+190, 336);
		}
		if (currentSelection == 9||selector.overlaps(upgrade3Rect)){
			batch.draw(upgradeSelectionImg,  167, 336-138);
		}
		if (currentSelection == 10||selector.overlaps(upgrade4Rect)){
		...
		main.font.draw(batch, Integer.toString((Space.player.fireFormLvl*50+Space.player.fireFormLvl*Space.player.fireFormLvl*Space.player.fireFormLvl)-Space.player.fireFormXp), 690, 392 - 64);
