...
public class NewWaveWindScreen implements Screen {
	private Texture glowImg;
	private Texture smallGlowImg;
	private Texture windScreenImg;
	...
	public NewWaveWindScreen(Space game) { 
		main = game;
		windScreenImg = main.windScreenImg;
	...
	public void render(float delta) {
		...
		if(!Gdx.input.isTouched()){
			...
			if (selector.overlaps(upgrade1Rect)){
				//Upgrade Fire Form Duration
				Space.player.upgradeWindDuration();
				selector.x = -400;
				selector.y = -240;
			}
			if (selector.overlaps(upgrade2Rect)){
				//Upgrade Fireball Damage
				Space.player.upgradeWindBladeDamage();
				selector.x = -400;
				selector.y = -240;
			}
			if (selector.overlaps(upgrade3Rect)){
				//Upgrade FireRate
				Space.player.upgradeWindBladeFireRate();
				selector.x = -400;
				selector.y = -240;
			}
			if (selector.overlaps(upgrade4Rect)){
				//Upgrade Health
				Space.player.upgradeWindFormVelocity();
				selector.x = -400;
				selector.y = -240;
			}
		}
		...
		if(Gdx.input.isKeyPressed(Keys.ENTER)){
			...
			if (currentSelection == 4)main.setScreen(main.mainMenu);
			}
		}
		...
		main.font.draw(batch, Integer.toString(Space.player.windFormLvl), 690, 392);
		main.font.draw(batch, Integer.toString(Space.player.windFormXp), 690, 392 - 32);
		main.font.draw(batch, Integer.toString((Space.player.windFormLvl*50+Space.player.windFormLvl*Space.player.windFormLvl*Space.player.windFormLvl)-Space.player.windFormXp), 690, 392 - 64);
		...
		main.font.draw(batch, Integer.toString(Space.player.windFormDurationCost), 220, 335); //Top Left
		main.font.draw(batch, Integer.toString(Space.player.windBladeDamageCost), 220+190, 335);//Top Right
		main.font.draw(batch, Integer.toString(Space.player.windBladeFireRateCost), 220, 335-138); //Bottom Left
		main.font.draw(batch, Integer.toString(Space.player.windFormVelocityCost), 220+190, 335-138);//Bottom Right
